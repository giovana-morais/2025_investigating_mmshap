...
def explain_ALM(
    ...
    def get_prediction(x):
        ...
        for i in range(masked_question_tokens.shape[0]):
            masked_prompt = (
                masked_question_tokens[i].unsqueeze(0).clone().detach().to("cuda:0")
            )
            ...
            masked_inputs["Audio"] = [masked_audio, 1.0]
    ...
    question_tokens = model.tokenizer.encode(
        entry["prompt"], bos=True, eos=False, out_type=str
    )
    ...
    for entry in questions:
        ...
        try:
            response = explain_ALM(
                entry=entry, audio_path=audio_url, model=model, args=args
            )
